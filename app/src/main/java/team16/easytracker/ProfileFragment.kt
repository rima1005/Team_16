...
class Profile : Fragment(R.layout.fragment_profile) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        currentWorker = DbHelper.loadWorker(workerId)!!
        val address : Address? = currentWorker?.addressId?.let { DbHelper.loadAddress(it) }
    ...
    fun updateProfile() {
        ...
        {
            val workerId = MyApplication.loggedInWorker!!.getId()
            DbHelper.updateWorker(workerId, firstNameSetting, lastNameSetting,
                    currentWorker.dateOfBirth, titleSetting, currentWorker.email,
                    phonePrefixSetting + phoneNumberSetting, currentWorker.createdAt,
                    DbHelper.saveAddress(streetSetting, postCodeSetting, citySetting))
        }
...
class ChangePasswordDialogFragment : DialogFragment() {
    ...
    fun validatePasswordChange() : Boolean {
        if(DbHelper.updateWorkerPassword(MyApplication.loggedInWorker!!.getId(), etOldPassword.text.toString(), etNewPassword.text.toString()))
        {
            return true;
        }
        else
